function [] = MakeFig_Sine_HeadFree_HeadvWing_Pos_Time(amp,root,figNum)
...
amp = 1:5;
%---------------------------------------------------------------------------------------------------------------------------------
filename = 'Sine_HeadFree_HeadvWing_Pos_Time'; % name of figure to save
...
for kk = 1:length(amp)
    for jj = 1:HeadFree{amp(kk)}.n.Freq
        ...
        if (subIdx-1)/HeadFree{amp(kk)}.n.Freq == round((subIdx-1)/HeadFree{amp(kk)}.n.Freq)
        	ylabel({[num2str(3.75*amp(kk)) '$^{\circ}$'],'Wing (V)'},'Interpreter','latex','FontSize',15)
            yticks(-10:5:10)
        else
            yticks(0)
            yticklabels('')
        end
        xData = HeadFree{amp(kk)}.HEAD.ALL.Pos{jj}(1*Fs:T*Fs,:);
        yData = HeadFree{amp(kk)}.WING.ALL.Pos{jj}(1*Fs:T*Fs,:);
