function [] = MakeFig_Sine_HeadFree_Head_Pos_Cohr(amp,root,figNum)
%% MakeFig_Sine_HeadFree_Head_Pos_Cohr:
%   INPUTS:
...
% figNum = 1;
%---------------------------------------------------------------------------------------------------------------------------------
filename = 'Sine_HeadFree_Head_Pos_Cohr'; % name of figure to save
...
for jj = 1:length(amp)
    for kk = 1:HeadFree{amp(jj)}.N{1,1}
        for ii = 1:HeadFree{amp(jj)}.N{1,3}
            ...
            h.med = plot( HeadFree{amp(jj)}.U{1,3}{1}(ii) , HeadFree{amp(jj)}.HEAD.COHR.Mag{kk}{ii} ,'*','Color',[0.5 0.5 0.5],...
                'LineWidth',1);      
        end
        subplot(1,length(amp),jj) ; hold on
        SE = 2*HeadFree{amp(jj)}.HEAD.FlySTD.COHR.Mag{kk}/1;
        SE = SE.*0;
        h.med = errorbar( HeadFree{amp(jj)}.U{1,3}{1} , HeadFree{amp(jj)}.HEAD.FlyMed.COHR.Mag{kk} , SE ,'-o',...
            'LineWidth',1);
    end
end
...
for jj = 1:length(amp)
    ...
    title([num2str(legList{jj}) '$^{\circ}$'],'Interpreter','latex','FontSize',15)
        SE = 2*HeadFree{amp(jj)}.HEAD.GrandSTD.COHR.Mag;
        h.med = errorbar( HeadFree{amp(jj)}.U{1,3}{1} , HeadFree{amp(jj)}.HEAD.GrandMed.COHR.Mag , SE ,'k','LineWidth',3);
