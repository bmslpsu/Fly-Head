function [] = MakeFig_Sine_HeadFree_Pat_Pos_Time(amp,root,figNum)
%% MakeFig_Sine_HeadFree_Pat_Pos_Time: time domain sinusoid plot for all amplitudes & frequencies
%   INPUTS:
...
% amp = 1:5;
%---------------------------------------------------------------------------------------------------------------------------------
filename = 'Sine_HeadFree_Pat_Pos_Time'; % name of figure to save
...
%             h.Trial = plot(HeadFree{amp(mm)}.PAT.Time{kk}{jj},HeadFree{amp(mm)}.PAT.Pos{kk}{jj},'Color',[0.5 0.5 0.5],'LineWidth',1); % trials
%             for ii = 1:length(h.Trial)
%                 h.Trial(ii).Color(4) = 0.25;
%             end
%             h.Fly = plot(HeadFree{amp(mm)}.PAT.FlyMed.Time{kk}(:,jj),HeadFree{amp(mm)}.PAT.FlyMed.Pos{kk}(:,jj),'LineWidth',2); % flys
%             h.Fly.Color(4) = 0.5;
%         end
%     end
    for jj = 1:HeadFree{amp(mm)}.n.Freq
        ...
        title([num2str(HeadFree{amp(mm)}.unq.Freq(jj)) ' Hz'],'Interpreter','latex','FontSize',15)
            [h.patch,h.med{jj,mm}] = PlotPatch(HeadFree{amp(mm)}.PAT.GrandMed.Pos(:,jj),HeadFree{amp(mm)}.PAT.GrandSTD.Pos(:,jj),...
                HeadFree{amp(mm)}.PAT.GrandMed.Time(:,jj),2,HeadFree{amp(mm)}.n.Fly,CList{amp(mm)},[0.4 0.4 0.6],0.5,2); % all flys
