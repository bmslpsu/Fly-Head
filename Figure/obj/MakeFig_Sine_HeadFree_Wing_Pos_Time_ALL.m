function [FIG] = MakeFig_Sine_HeadFree_Wing_Pos_Time_ALL()
%% MakeFig_Sine_HeadFree_Wing_Pos_Time_ALL:
%   INPUTS:
%       -
%   OUTPUTS:
%       FIG     :   figure handle
...
%%
figNum = 1;
filename = 'Sine_HeadFree_Wing_Pos_Time_ALL'; % name of figure to save
...
for ww = 1:nAmp % amplitudes 
    ...
    for jj = 1:HeadFree{ww}.N{1,3} % frequencies
        freq = HeadFree{ww}.U{1,3}{1}(jj);
     	subplot(HeadFree{ww}.N{1,3},1,pp)  ; hold on ; xlim([0 10]) ; ylim(3*[-1 1])

     	title([num2str(freq) ' Hz'],'Interpreter','latex','FontSize',15)
        
       	ylabel(['$\Delta$ WBA (V)'],'Interpreter','latex','FontSize',12)
