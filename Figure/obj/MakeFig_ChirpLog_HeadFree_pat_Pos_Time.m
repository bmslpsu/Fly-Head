function [FIG] = MakeFig_ChirpLog_HeadFree_pat_Pos_Time()
%% MakeFig_ChirpLog_HeadFree_pat_Pos_Time.m:
%   INPUTS:
%       -
%   OUTPUTS:
%       FIG     :   figure handle
...
filename = 'ChirpLog_HeadFree_pat_Pos_Time'; % name of figure to save
        ...
        for ii = 1:1
            subplot(HeadFree.N{1,3},1,pp) ; hold on ; xlim([0 20]) ; ylim(20*[-1 1])
                h.Trial = plot(HeadFree.TRIAL{kk,jj}{ii,catIdx}.Time,HeadFree.TRIAL{kk,jj}{ii,catIdx}.X(:,xIdx),...
                    '-','Color',[0.5 0.5 0.5 1],'LineWidth',3);
        end
    end
    pp = pp + 1;
end
