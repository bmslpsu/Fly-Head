function [FIG] = MakeFig_ChirpLog_HeadFree_pat2wing_Pos_BODE()
%% MakeFig_ChirpLog_HeadFree_pat2wing_Pos_BODE:
%   INPUTS:
%       -
%   OUTPUTS:
%       FIG     :   figure handle
...
filename = 'ChirpLog_HeadFree_pat2wing_Pos_BODE'; % name of figure to save
...
for jj = 1:HeadFree.N{1,3}
    ...
    ax = subplot(2,HeadFree.N{1,3},pp+HeadFree.N{1,3}) ; hold on ; xlim([0.1 12]) ; ylim(pi*[-1 1])
        h.patch = PlotPatch(medfilt1(HeadFree.GRAND{jj,catIdx}.CircMean{9}{3}(:,xIdx),5),medfilt1(HeadFree.GRAND{jj,catIdx}.CircSTD{9}{3}(:,xIdx),5),...
            HeadFree.GRAND{jj,catIdx}.Mean{2}{1}(:,xIdx),3,HeadFree.N{1,1},'k',[0.4 0.4 0.6],0.5,2);
