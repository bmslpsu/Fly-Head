function [FIG] = MakeFig_Chirp_Sine_HeadFree_err2wing_BODE()
%% MakeFig_Chirp_Sine_HeadFree_err2wing_BODE:
%   INPUTS:
%       -
%   OUTPUTS:
%       FIG     : figure handle
...
%%
figNum = 1;
filename = 'Chirp_Sine_HeadFree_err2wing_BODE'; % name of figure to save
catIdx = 6;
xIdx = 1;
...
% Grand Stats
for ww = 1:nAmp % amplitudes
    ax1 = subplot(2,1,1) ; hold on ; xlim([0.1 12]) ; ylim(0.3*[0 1]) ; title([num2str(Amp) , char(176)])
        ax1.FontSize = 12;
    ...
    HeadFree{nAmp+1}.GRAND{3,catIdx}.Mean{2}{1}(:,xIdx),3,HeadFree{nAmp+1}.N{1,1},'r',[0.4 0.4 0.6],0.5,2);
