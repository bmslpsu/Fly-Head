function [FIG] = MakeFig_ChirpLog_HeadFree_pat_head_wing_Pos_Time()
%% MakeFig_ChirpLog_HeadFree_pat_head_wing_Pos_Time.m:
%   INPUTS:
%       -
%   OUTPUTS:
%       FIG     :   figure handle
...
filename = 'ChirpLog_HeadFree_pat_head_wing_Pos_Time'; % name of figure to save
...
% Grand Stats
for jj = 3
    subplot(2,1,1) ; hold on ; xlim([0 20])
...
%             HeadFree.GRAND{jj,2}.Mean{2}{5},2,HeadFree.N{1,1},'b',[0.4 0.4 0.6],0.5,3);
	...
	yyaxis left
        ...
        h.patch = PlotPatch(HeadFree.GRAND{jj,2}.Mean{2}{6}(:,xIdx),HeadFree.GRAND{jj,2}.STD{2}{6}(:,xIdx),...
            HeadFree.GRAND{jj,2}.Mean{2}{5},3,HeadFree.N{1,1},'b',[0.4 0.4 0.6],0.5,3);
        
    subplot(2,1,2) ; hold on ; xlim([0 20])
    title([num2str(HeadFree.U{1,3}{1}(jj)) char(176)],'FontSize',15)
