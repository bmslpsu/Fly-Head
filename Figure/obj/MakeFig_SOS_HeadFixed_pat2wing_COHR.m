function [FIG] = MakeFig_SOS_HeadFixed_pat2wing_COHR()
%% MakeFig_SOS_HeadFixed_pat2wing_BODE: BODE head position for SOS
%   INPUTS:
...
filename = 'SOS_HeadFixed_pat2wing_BODE'; % name of figure to save
...
ax2 = subplot(2,1,2) ; hold on
    ...
    ax2.XLabel.Color = 'k';
 	ax2.YLabel.String = ['Pat to Wing Phase (' char(176) ')'];
    ax2.YLabel.FontSize = ax1.XLabel.FontSize;
    ax2.YTick = -300:60:180;
    
    phase = HeadFixed.GRAND{1,catIdx}.CircMean{9}{6}(:,xIdx);
    phase(phase>1) = phase(phase>1) - 2*pi;
    
    errorbar(HeadFixed.GRAND{1,catIdx}.Mean{2}{4},rad2deg(phase),...
        rad2deg(2*HeadFixed.GRAND{1,catIdx}.CircSTD{9}{6}(:,xIdx)),'-or','LineWidth',3);
