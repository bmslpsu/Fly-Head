function [FIG] = MakeFig_ChirpLog_HeadFree_pat2wing_BODE_ALL_new()
%% MakeFig_ChirpLog_HeadFree_pat2wing_BODE_ALL_new:
%   INPUTS:
%       -
%   OUTPUTS:
%       FIG     : figure handle
...
FREQ = HeadFree.GRAND{jj,stim2wing}.Mean{2}{1}(:,xIdx);
VEL	 = AMP*2*pi*FREQ;

GAIN  = HeadFree.GRAND{jj,stim2wing}.Mean{2}{2}(:,xIdx);
PHASE = rad2deg(HeadFree.GRAND{jj,stim2wing}.CircMean{7}{3}(:,xIdx));
GSTD  = HeadFree.GRAND{jj,stim2wing}.STD{2}{2}(:,xIdx);
PSTD  = rad2deg(HeadFree.GRAND{jj,stim2wing}.CircSTD{7}{3}(:,xIdx));
...
for jj = 1:HeadFree.N{1,3} % amplitudes
    ...
    FREQ(:,jj)  = HeadFree.GRAND{jj,stim2wing}.Mean{2}{1}(:,xIdx);
    VEL(:,jj) 	= AMP(:,jj)*2*pi*FREQ(:,jj);
    GAIN(:,jj)  = HeadFree.GRAND{jj,stim2wing}.Mean{2}{2}(:,xIdx);
    PHASE(:,jj) = rad2deg(HeadFree.GRAND{jj,stim2wing}.CircMean{7}{3}(:,xIdx));
 	GSTD(:,jj)  = HeadFree.GRAND{jj,stim2wing}.STD{2}{2}(:,xIdx);
    PSTD(:,jj)  = rad2deg(HeadFree.GRAND{jj,stim2wing}.CircSTD{7}{3}(:,xIdx));
