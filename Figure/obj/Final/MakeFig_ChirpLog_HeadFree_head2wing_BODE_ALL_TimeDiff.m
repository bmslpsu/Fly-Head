...
for jj = 1:HeadFree.N.Amp % amplitudes
    ...
    FREQ_mean = HeadFree.GRAND{jj,catIdx}.Mean{2}{17}(:,xIdx); % complex frequency domain
    FREQ_std = HeadFree.GRAND{jj,catIdx}.Mean{2}{17}(:,xIdx); % complex frequency domain
    Gain = HeadFree.GRAND{jj,catIdx}.Mean{2}{2}(:,xIdx);
    GainSTD = HeadFree.GRAND{jj,catIdx}.STD{2}{2}(:,xIdx);
    Phase = HeadFree.GRAND{jj,catIdx}.CircMean{7}{3}(:,xIdx);
    PhaseSTD = HeadFree.GRAND{jj,catIdx}.CircSTD{7}{3}(:,xIdx);
    
    [newFv,newFREQ_mean,~,~] = DFT_bin(Fv, FREQ_mean, freq_bins);
    [~,newFREQ_std,~,~]      = DFT_bin(Fv, FREQ_std, freq_bins);
    
	[~,newGain,~,~]      = DFT_bin(Fv, Gain, freq_bins);
    [~,newGain_std,~,~]  = DFT_bin(Fv, GainSTD, freq_bins);
	[~,newPhase,~,~]     = DFT_bin(Fv, Phase, freq_bins);
    [~,newPhase_std,~,~] = DFT_bin(Fv, PhaseSTD, freq_bins);
...
%     PSTD(:,jj) 	= rad2deg(angle(newFREQ_std));
	...
	GAIN(:,jj)	= newGain;
    PHASE(:,jj)	= rad2deg(newPhase);
    GSTD(:,jj) 	= newGain_std;
    PSTD(:,jj) 	= rad2deg(newPhase_std);
...
set(ax(1,:),'YLim',[0 0.6],'YTick',0:0.2:0.6)
set(ax(2:3,:), 'YLim', [-300 180], 'YTick', -300:60:180)
set(ax(4,:), 'YLim', [-180 0], 'YTick', -180:60:0)
linkaxes(ax,'x')
