...
for kk = 1:N.Free.file
    ...
    WBA.Free(kk,1) = Wing.L;
    WBA.Free(kk,2) = Wing.R;
...
for kk = 1:N.Fixed.file
    ...
    WBA.Fixed(kk,1) = Wing.L;
    WBA.Fixed(kk,2) = Wing.R;
...
for kk = 1:size(h,1)
   patch(h{kk,1},h{kk,2},[0.35 0 0.65]);
end
...
ax1 = subplot(2,1,1);
imagesc(vid.Free_med{7})
% imagesc(vid.Free_all)
axis off equal
title('Free')

ax2 = subplot(2,1,2);
imagesc(vid.Fixed_med{3})
% imagesc(vid.Fixed_all)
axis off equal
title('Fixed')
