function [FIG] = MakeFig_ChirpLog_HeadFree_pat2head_Pos_COHR()
%% MakeFig_ChirpLog_HeadFree_pat2head_Pos_COHR:
%   INPUTS:
%       -
%   OUTPUTS:
%       FIG     :   figure handle
...
filename = 'ChirpLog_HeadFree_pat2head_Pos_COHR'; % name of figure to save
...
for jj = 1:HeadFree.N{1,3}
    ...
    title([num2str(HeadFree.U{1,3}{1}(jj)) char(176)],'FontSize',15)
        h.patch = PlotPatch(HeadFree.GRAND{jj,catIdx}.Mean{2}{7}(:,xIdx),HeadFree.GRAND{jj,catIdx}.STD{2}{7}(:,xIdx),...
            HeadFree.GRAND{jj,catIdx}.Mean{2}{8}(:,xIdx),3,HeadFree.N{1,1},'k',[0.4 0.4 0.6],0.5,2);
               
        if pp==1
            ylabel('Pattern to Head Coherence','Interpreter','latex','FontSize',15)
       	else
            yticks('')
        end
