function [MOV] = make_montage_rigid_head_wing_pat_Chirp(rootdir,rootpat,vidFs,export)
%% make_montage_rigid_head_wing_pat_Chirp: makes movie for fly in rigid tether
%
...
PATH.head_track     = fullfile(PATH.vid,'Angles');
PATH.beninfly_track	= fullfile(PATH.vid,'tracked_head_wing');
...
PAT.norm = 15*chirp(FLY.int_time, 0.1, 20, 12, 'logarithmic');

% [b,a]       = butter(3, 60/(FLY.Fs/2), 'low');
...
FLY.head_length = 38;
FLY.head_hinge = [params.gui.head.hinge.x  , 2 + params.gui.head.hinge.y];
...
set(ax(4), 'YLim', w_ylim*[-1 1], 'YTick', (w_ylim-5)*[-1 0 1])
...
for jj = 1:FLY.nframe % for each frame
    if expframe(jj) % if we want to display this frame
        ...
        Frame.raw = 1.1*median(FLY.raw(:,:,win),3); % current raw frame median across frames
