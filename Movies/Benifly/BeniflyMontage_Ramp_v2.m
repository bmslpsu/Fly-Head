function [MOV] = BeniflyMontage_Ramp_v2(rootdir,rootpat,vidFs,export)
%% BeniflyMontage_Ramp_v2:  makes movie for fly in rigid tether
...
subplot(12,1,9:12)  ; cla ; hold on ; h.head = animatedline('Color','c','LineWidth',linewidth); % for pattern angle
...
for jj = 1:n_frame % for each frame
    ...
    % Head plot
    ax(1) = subplot(12,1,9:12) ; hold on
...
%     addpoints(h.wba, Fly.time(jj), Fly.wba(jj))
    ...
    set(ax, 'YLim', 20*[-1 1], 'YTick', [-15 0 15])
    set(ax, 'XTick', 0:2:round(Fly.time(end)))
    set(ax, 'FontSize', 12)
