function [] = SacdDetect(vel,time,N,stim)
...
N               = 2.5;
n               = length(t_v);
Ts              = mean(diff(t_v));                  % sampling time
Fs              = 1/Ts;                             % sampling frequency
time.data    	= (0:Ts:t_v(end))';                 % normalized time
pat.pos.data 	= 60*time.data;                     % stimulus data
pat.vel.data  	= [0 ; diff(pat.pos.data)/Ts];    	% velocity data
pat.avel.data  	= abs(pat.vel.data);                % velocity data
[b,a]           = butter(2,25/(Fs/2));              % low-pass filter coefficents
pos.data        = filtfilt(b,a,hAngles);            % position data
vel.data        = [0 ; diff(pos.data)/Ts];          % velocity data
