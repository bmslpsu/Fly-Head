function [FIG] = MakeFig_Static_Head_Saccade_v2()
%% MakeFig_Static_Head_Saccade_v2:
%   INPUTS:
%       -
%   OUTPUTS:
%       FIG     :   figure handle
...
for jj = 1:N{1,3}
    ...
    ax(jj).Title.String = [num2str(Wave(jj)) ' (' char(176) '/s)'];
    ax(jj).Title.Color = 'k';
...
set([YLabelHC{:}], 'String', ['Head Position (' char(176) '/s)'])
linkaxes(ax(1:3),'y')
linkaxes(ax(4:6),'y')
linkaxes(ax,'x')
...
for jj = 1:N{1,3}
    ...
    plot([-100 100],-sign(Wave(jj))*300*[1 1],'--','Color',[0.5 0.5 0.5],'LineWidth',1)
...
for jj = 6
	...
	[h.patch(jj),h.grand(jj)] = PlotPatch(SACCADE.HeadStats.Velocity(jj).Median(span), SACCADE.HeadStats.Velocity(jj).STD(span), ...
        1000*SACCADE.HeadStats.Time(jj).Median(span), 1, 1, CC{jj}, [0.7 0.7 0.7], 0.4, 3);
    delete(h.patch(jj))
...
for jj = 1:N{1,3}
    ...
    hp(jj) = plot([0 10], repmat(Wave(jj),1,2), '--', 'Color', [0.5 0.5 0.5], 'LineWidth', 1.5);
end
...
SS = [6,7,15,17,19,24,26,25,27,23];
YY = {  ['Duration (ms)'],...
        ['Amplitude (' char(176) ')'],...
...
for ww = 1:length(SS)
    ...
    axis tight
        ...
        if any(SS(ww)==[6,7,15,17,19,24,26,25,27,23])
            data = abs(data);
        end
...
%     plot(GRAND{jj,rmvIdx}.Mean{1}{5}, GRAND{jj,rmvIdx}.Mean{1}{6}(:,xIdx), 'Color', CC{jj},'LineWidth',3)
	PlotPatch(GRAND{jj,rmvIdx}.Mean{1}{6}(:,xIdx), GRAND{jj,rmvIdx}.STD{1}{6}(:,xIdx), ...
        GRAND{jj,rmvIdx}.Mean{1}{5}, 1, 1, CC{jj}, [0.7 0.7 0.7], 0.4, 3);
    
    plot(GRAND{jj,rmvIdx}.Mean{1}{5},Stim(:,jj), '-', 'Color', 'k', 'LineWidth',1.5)
end
set(ax,'XLim',[0 10])
...
for jj = 1:N.vel
    ...
    ax(jj).Title.String = [num2str(Wave(jj)) ' (' char(176) '/s)'];
    ax(jj).XLabel.String = 'Time(s)';
    ax(jj).YLabel.String = ['Head Velocity (' char(176) ')'];
%     for kk = 1:N.fly
%         for ii = 1:size(TRIAL{kk,jj},1)
%             h.trial = plot(TRIAL{kk,jj}{ii,rmvIdx}.Time, TRIAL{kk,jj}{ii,rmvIdx}.X(:,xIdx), 'Color', 0.5*CC{jj});
%             h.trial.Color(4) = 0.2;
...
%         plot(FLY{jj}{kk,rmvIdx}.Mean{5}, FLY{jj}{kk,rmvIdx}.Mean{6}(:,xIdx), 'Color', 0.7*CC{jj},'LineWidth',1)
%     end
    plot(GRAND{jj,rmvIdx}.Mean{1}{5}, GRAND{jj,rmvIdx}.Mean{1}{6}(:,xIdx), 'Color', CC{jj},'LineWidth',0.5)
    plot(GRAND{jj,rmvIdx}.Mean{1}{5}, Wave(jj)*ones(length(GRAND{jj,rmvIdx}.Mean{1}{5}),1), '--', 'Color', 'k', 'LineWidth',1.5)
end
set(ax,'XLim',[0 10])
set(ax,'YLim',100*[-1 1])
set(ax([1:4,6]),'XColor','none')
