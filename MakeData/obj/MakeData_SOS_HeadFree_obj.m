...
ErrFV	= GRAND{errIdx}.Mean{1}{7}(:,xIdx);
ErrMag	= GRAND{errIdx}.Mean{1}{8}(:,xIdx);
...
replay_trial = Fly(Error(:,minIdx),Time,[],IOFreq,[]);
...
subplot(2,1,1) ; hold on
    ...
    h.ref = plot(Time,RefMean,'k','LineWidth',1);
    [~,h.ErrMean] = PlotPatch(ErrMean, ErrSTD, Time, 2, N{1,1}, 'b' ,[0.4 0.4 0.6], 0.4 , 1);
    h.ErrTrial = plot(Time,Error(:,minIdx),'r','LineWidth',1);
%     leg = legend([h.ref,h.ErrMean,h.ErrTrial],'Reference','Mean Error','Replay Trial Error');
%     leg.Box = 'off';
...
subplot(2,1,2) ; hold on
    ...
    xlabel('Frequency (Hz)')
    ylabel(['Angle (' char(176) ')'])
    h.ref = plot(TRIAL{1}{1,1}.Fv, TRIAL{1}{1,1}.Mag(:,xIdx), 'k', 'LineWidth', 1);
    h.ErrMean = plot(ErrFV, ErrMag, 'b', 'LineWidth', 1);
    h.ErrTrial = plot(replay_trial.Fv, replay_trial.Mag(:,1), 'r', 'LineWidth', 1);
